 ...
 */
/** Normalise a groups arg to a clean, deduped string[] with fuel dropped. */
function normGroups(groups?: string[] | null): string[] {
  if (!Array.isArray(groups)) return []
  const seen = new Set<string>()
  for (const g of groups) {
    const s = String(g ?? "").trim()
    if (s && !EXCLUDED_GROUPS.includes(s)) seen.add(s)
  }
  return [...seen]
}

export function groupFilter(groups?: string[] | null) {
  const list = normGroups(groups)
  return list.length > 0 ? { $in: list } : { $nin: EXCLUDED_GROUPS }
}

/** Stable cache key for a set of selected groups (null = all groups). */
export function groupKey(groups?: string[] | null): string | null {
  const list = normGroups(groups)
